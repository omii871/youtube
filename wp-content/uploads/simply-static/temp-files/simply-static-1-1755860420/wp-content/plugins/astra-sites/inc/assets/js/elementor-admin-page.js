...
( function ( $ ) {
	...
	$.fn.isInViewport = function () {
		...
		return elementBottom &gt; viewportTop &amp;&amp; elementTop  0 ) {
					...
					action_for_add_section = action_for_add_section.replace(
						'</p><div class="elementor-add-section-drag-title',
						stylesheet +
							'&lt;div class=" elementor-add-section-area-button elementor-add-ast-site-button white_label_class title="' +
		...
		insertBlockOnRefresh() {
			...
			// Get the value of the " blockid parameter const type="urlObj.searchParams.get(" : if remotest data="astraElementorSites[" templatemodel="new" backbone.model gettitle return this.requestinsert this.afterinsert this.beforeinsert this.failedcallback this.removeaddedparams _populate_color_filters: function object.keys astraelementorsites.astra_blocks let template="wp.template(" var colorfilters="[];" for block_id in colorfilters.includes colorfilters.push colorfilters.length .show .html _paginateblocks: astraelementorsitesadmin.type="=" undefined .find .offset>=
						$elscope
		...
		_bulkPluginInstallActivate: function () {
			...
			if ( activate_plugins.length ' ).html( input_string ).text();
			var input_string =
